...
float ball_angle_180_second = 0;
float distance_pixels = 0;
float distance_pixels_second = 0;
float distance_pixels_protect = 0;
float ball_angle = 0;
float ball_distance = 0;
float goal_angle = 0;
float ball_angle_second = 0;
float ball_distance_second = 0;
float goal_angle_second = 0;
...
const int esc_pin = 6;
bool ball_seen_openmv_second = false;
...
void loop()
{
    ...
    if (pinmotor == 0)
    {
        ...
        //------------------------ Second OpenMV via UART ------//
        if (Serial6.available())
        {
            String camStringSecond = Serial1.readStringUntil('\n');
            ball_distance_second = camStringSecond.toFloat();
            ball_angle_second = camStringSecond.substring(camStringSecond.indexOf(' ') + 1).toFloat();
            goal_angle_second = camStringSecond.substring(camStringSecond.indexOf(' ', camStringSecond.indexOf(' ') + 1) + 1, camStringSecond.lastIndexOf(' ')).toFloat();
            distance_pixels_second = camStringSecond.substring(camStringSecond.lastIndexOf(' ') + 1).toFloat();
            ball_seen_openmv_second = (ball_distance_second != 0 || ball_angle_second != 0);
              Serial.println(camStringSecond);
        }
        ...
        if (ball_angle_second < 180)
        {
            ball_angle_180_second = -ball_angle_second;
        }
        else if (ball_angle_second > 180)
        {
            ball_angle_180_second = 360 - ball_angle_second;
        }
        ball_angle_180_second = ball_angle_180_second * (-1);
